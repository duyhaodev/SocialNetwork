...
export function FollowingFeedTab() {
  ...
  const fetchFollowingFeed = useCallback(
    async (pageNum, { force = false } = {}) => {
      ...
      setLoading(true);
      try {
        const res = await postApi.getFollowingFeed({ page: pageNum, size: SIZE });
  ...
  useEffect(() => {
    if (!initialized) {
      setInitialized(true);
      fetchFollowingFeed(0);
    }
  }, [initialized, fetchFollowingFeed]);
  ...
  useEffect(() => {
    ...
    setPage(0);
    setHasMore(true);
    fetchFollowingFeed(0, { force: true });
  ...
  }, [hasMore, loading, page, fetchFollowingFeed]);
