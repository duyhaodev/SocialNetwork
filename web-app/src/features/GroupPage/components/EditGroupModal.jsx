...
export function EditGroupModal({ isOpen, onClose, onSuccess, group }) {
  ...
  useEffect(() => {
    if (group && isOpen) {
      reset({
        ...
        requiresApproval: group.requiresApproval !== undefined ? group.requiresApproval : true
      });
  ...
  const onSubmit = async (data) => {
    setIsLoading(true);
    try {
      let finalCoverUrl = group.coverImageUrl || "";
      ...
      const res = await groupApi.updateGroup(group.id, payload);
      if (res.code === 1000) {
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Thay đổi thông tin và quy định của nhóm.
          </DialogDescription>
        </DialogHeader>
