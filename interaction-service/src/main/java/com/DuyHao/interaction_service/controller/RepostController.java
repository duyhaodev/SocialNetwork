...
public class RepostController {
    ...
    @PostMapping("/{postId}")
    public ApiResponse<RepostResponse> toggleRepost(@PathVariable String postId, @AuthenticationPrincipal Jwt jwt) {
        String userId = jwt.getSubject();
        var result = repostService.toggleRepost(postId, userId);
        return ApiResponse.<RepostResponse>builder().result(result).build();
    }
}
