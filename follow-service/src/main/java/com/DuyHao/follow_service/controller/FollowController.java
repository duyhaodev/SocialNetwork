...
public class FollowController {
    ...
    public ResponseEntity<?> toggleFollow(@PathVariable String followingId, Authentication auth) {
        ...
        if (isFollowing) response = followService.unfollowUser(followerId, followingId);
        else response = followService.followUser(followerId, followingId);

        return ResponseEntity.ok(response);
    }

    @GetMapping("/status/{followingId}")
    public boolean checkFollowing(@PathVariable String followingId, Authentication auth) {
